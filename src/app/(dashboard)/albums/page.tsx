...
const albums = () => {
    ...
    const fetchAlbums = async (labelId: string) => {
        try {
            const response = await apiGet(`./api/albums/getAlbums?labelid=${labelId}`)
            if (response.success) {
    ...
    return (
        <div className=' w-100'>
            ...
            <div className={`bg-white p-3 border rounded mt-5 ${Style.musicListContainer}`}>
                ...
                <div className={Style.musicList}>
                    {
                        albumList ?
                            <AlbumDataTable data={albumList} /> : <h3 className='text-center mt-4'>No Albums found</h3>
                    }
