...
const copyrights = () => {
  ...
  const fetchAllCopyrights = async (labelId: any) => {
    setIsLoading(true)
    try {
      const response = await apiGet(`/api/copyright/getCopyrights?labelid=${labelId}`)
      if (response.success) {
  ...
  return (
    <div className="w-full h-full p-6 bg-white rounded-sm" style={{ minHeight: "90vh" }} >
      ...
      <div className="flex justify-between items-center mt-3">
        <h3 className="text-3xl font-bold mb-2 text-blue-500">All Copyrights</h3>
        <Button onClick={() => setIsModalVisible(true)}>New Copyrights</Button>
      </div>

      {
      copyrights && <div className="bg-white p-3">
      ...
      {
        !copyrights && !isLoading && <h5 className="text-2xl mt-5 pt-3 text-center">No Record Found</h5>
      }
